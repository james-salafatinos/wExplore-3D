...
function init() {
          ...
          .then((DATA) => {
            ...
            loader.load(FONT_TYPEFACE, function (font) {
              try {
                for (
                  let i = 0;
                  i <= Math.min(DATA.nodes.length, graphParams.nodeCount);
                  i += 1
                ) {
                  let label = DATA.nodes[i]["label"];

                  let size = 0.4; //DATA.nodes[i]['attributes']['importance']//.2
                  let x =
                    graph_config["origin"][0] + DATA.nodes[i]["x"] / SCALE;
                  let y =
                    graph_config["origin"][1] + DATA.nodes[i]["y"] / SCALE;
                ...
                console.log(
                  "Error, unable to load label (probably an out of bounds error)"
                );
              }
  ...
  let edgePlot = function (DATA, graph_config) {
    ...
    for (
      let i = 0;
      i <= Math.min(DATA.edges.length, graphParams.edgeCount);
      i += 1
    ) {
  ...
  let labelPlot = function (font, label, size, x, y, z, userData) {
    ...
    //## ADDS BACKGROUND
    // let background = createTextBackground(x, y + _h / 3, z + 0.9, _w, _h, _d);
  ...
  let createFrame = function (url, _x, _y, _z) {
    ...
    let frameScale = new THREE.Vector3(0.01, 0.01, 0.01);
...
function animate() {
  ...
  if (controls.isLocked === true) {
    ...
    if (moveForward || moveBackward)
      velocity.z -= direction.z * flyParams.flySpeed * delta;
    if (moveLeft || moveRight)
      velocity.x -= direction.x * flyParams.flySpeed * delta;
    if (moveUp || moveDown)
      velocity.y -= direction.y * flyParams.flySpeed * delta;
